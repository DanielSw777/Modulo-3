...
const Cart = () => {
    ...
    const incrementProductCount = async (id) => {
        try {
            ...
            const response = await fetch(`https://modulo-3-backend-aj6p2biq3-daniel777.vercel.app/api/v1/cart/aumentar/${id}`, {
                method: "PUT",
                headers: {
    ...
    const decrementProductCount = async (id) => {
        try {
            const token = localStorage.getItem("access");
            const response = await fetch(`https://modulo-3-backend-aj6p2biq3-daniel777.vercel.app/api/v1/cart/disminuir/${id}`, {
                method: "PUT",
                headers: {
                    "Authorization": `Bearer ${token}`
                }
            ...
            <article key={producto._id} className="main__product" data-id={producto._id}>
                <div className="main__container-img-title">
                    <div className="main__container-product-img">
                        <img className="main__product-img" src={`https://modulo-3-backend-aj6p2biq3-daniel777.vercel.app/${producto.image}`} alt={producto.name} />
                    </div>
